...
import { getMenuItems, saveMenuItems, MenuItem } from "@/lib/menu-data";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editing) {
      updated = items.map((item) =>
        item.id === editing.id
          ? { ...item, name, description, price: parseFloat(price), category, image }
          : item
      );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-lg border border-input bg-background text-foreground font-body text-sm file:mr-4 file:py-1 file:px-3 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-ocean/10 file:text-ocean"
                />
              </div>
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-2 rounded-lg border border-input bg-background text-foreground font-body focus:ring-2 focus:ring-ocean focus:outline-none resize-none"
                />
              </div>
            </div>
            {image && (
              <div className="mt-4">
                <img src={image} alt="Preview" className="h-32 w-32 object-cover rounded-lg border border-border" />
              </div>
            )}
            <div className="flex gap-3 mt-6">
              <button
                type="submit"
                className="bg-ocean text-ocean-foreground px-6 py-2.5 rounded-lg font-body font-semibold hover:opacity-90 transition-opacity shadow-md"
              >
              ...
              <button
                type="button"
                onClick={resetForm}
                className="bg-muted text-muted-foreground px-6 py-2.5 rounded-lg font-body font-semibold hover:bg-muted/80 transition-colors"
              >
                Cancel
              </button>
            </div>
            ...
            <div key={item.id} className="flex items-center gap-4 bg-card rounded-lg p-4 shadow-sm border border-border">
              ...
              <div className="flex gap-2 flex-shrink-0">
                <button
                  ...
                  className="p-2 rounded-lg bg-ocean/10 text-ocean hover:bg-ocean/20 transition-colors"
                >
